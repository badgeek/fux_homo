...
ifeq ($(UNAME),Darwin)
 CPPFLAGS += -DDARWIN
 INCLUDES +=  -I/sw/include -I$(GEM_DIR)/src -I$(GEM_DIR)/src/openGL -I$(PD_DIR)/src -I$(PD_DIR) -I./
 LDFLAGS = -c -arch i386 
 LIBS =  -lm
 EXTENSION = pd_darwin
endif
...
all:
	...
	g++ -o $(SOURCES).$(EXTENSION) -undefined dynamic_lookup -arch i386 -dynamiclib -mmacosx-version-min=10.3 -undefined dynamic_lookup -framework QuickTime -framework Carbon -framework AGL -framework OpenGL -arch i386 ./*.o -L/sw/lib -lstdc++ -ldl -lz -lm -lpthread -L$(PD_DIR)/bin -L$(PD_DIR)
	rm -fr ./*.o
deploy:
